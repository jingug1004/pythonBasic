 ...
 * PROGRAMMER    :                                                                          
=================================================================================================*/
#include "lib::boa_message_script.js";
...
var DeleteCount       ;
var ENTER_KEY = 13;
//var MAIL_URL = "http://210.90.29.17/user/mbrSendMail_test.html";					// 홈페이지 회원에게 메일 발송
//var MAIL_URL = "http://210.90.29.17/user/mbrSendMail.html";						// 홈페이지 회원에게 메일 발송
var MAIL_URL = "http://210.90.29.34:8080/ems70/sendMailToKboatChuljuClients.jsp";	// 홈페이지 회원에게 메일 발송
...
function fnc_FormLoadSetting(obj, sFORMCAPTION, sFORMLOCATION, sBUTTONLIST, sUSERAUTH) {
	...
	obj.Window.Title    = sFORMCAPTION + "(" + sFormId + ")";          //화면 Title Display
	obj.stFormTitle     = sFORMCAPTION + "(" + sFormId + ")";          //화면 정보 Display
	if (obj.stFormTitle.width < 300) obj.stFormTitle.width = 300;
...
function fnc_FileRead(httpfileObj, sLocalFileName, sServerFileName, nFileSize, nRow, strCol, dsObj) {
    ...
    var sRemoteUrl;
	sRemoteUrl = GBL_FILE_UPDNLOAD_URL+"?mode=download";
...
function fnc_Result(errCode, errMsg) {
    ...
    if ( !fnc_IsNull(SearchCount) ) {
		fnc_SetStatus(SNIS_BOA_0003, SearchCount);
        //fnc_Message(SNIS_BOA_0003, "N", SearchCount);
        SearchCount = NULL;
        return true;
    }

    if ( !fnc_IsNull(SaveCount) && !fnc_IsNull(DeleteCount) ) {
        fnc_Message(SNIS_BOA_0006, "N", SaveCount, DeleteCount);
		fnc_SetStatus(SNIS_BOA_0006, SaveCount, DeleteCount);
        SaveCount = NULL;
        DeleteCount = NULL;
    } else {
		if ( !fnc_IsNull(SaveCount) ) {
			fnc_SetStatus(SNIS_BOA_0004, SaveCount);
			//fnc_Message(SNIS_BOA_0004, "N", SaveCount);
			SaveCount = NULL;
		}
	
		if ( !fnc_IsNull(DeleteCount) ) {
			fnc_SetStatus(SNIS_BOA_0005, DeleteCount);
			//fnc_Message(SNIS_BOA_0005, "N", DeleteCount);
			DeleteCount = NULL;
		}
	}
...
function fnc_IsMandatory(controlID, controlName)
{
    ...
    {
        fnc_Message(SNIS_COM_1035, "Y", controlName);
		...
		if ( DataSet.GetRowType(i) != "normal" ) {
            ...
            for ( j = 0; j < MandatoryColID.length(); j++ ) {
                ...
                if ( fnc_IsNull(DataSet.GetColumn(i, MandatoryColID[j])) ) {
					...
					fnc_Message(SNIS_BOA_0001, "N", i + 1, MandatoryColName[j]); 
...
*                값이 없는 경우는 -1을 return
************************************************************************/
function fnc_GetLikeValue(DataSet, strColID, strValue, nStartRow) {
    var resultRow = -1;
    ...
    // 해당값을 리턴
    for ( i = nStartRow; i < DataSet.GetRowCount(); i++ ) {
        if ( substr(toUpperCase(trim(DataSet.GetColumn(i ,strColID))), 0, length(strValue)) == strValue ) {
            resultRow = i;
            break;
        }
    }
...
**************************************************************************************************/
function fnc_SendMessage(sendUser, recvUser, title, linkUrl, desc, compId, systemId) {
    ...
    var ServerURL = GBL_SERVER_URL+"miInstall/xssoLogin.jsp?MENUID=";
    if (systemId == "CANBOA") { //경정훈련관리
		ServerURL = "http://canboa.kboat.or.kr/canboa/miInstall/xssoLogin.jsp?MENUID=";
    }
...
//SQL 방식 Report 호출
function fnc_SQLRPTPrint(sSubUrl, sRptNm, sRptDrct, argsKeys, argsValues) {
    var sFileName  = "";
    var iWidth     = 1014;
    var iHeight    = 630;  
			
	if (sRptDrct == 0){ //수직
		sFileName  = "common::comVrtSQLRptViewer.xml";
		iWidth  = 760;  		
	} 			
	
	Dialog(sFileName, " sSubUrl="  + quote(sSubUrl) +
	                  " sRptNm="   + quote(sRptNm) + 
	                  " argsKeys="   + quote(argsKeys) +
	                  " argsValues="   + quote(argsValues) + 
...
function fnc_SendMailList(subject, contents) {
	var sAttr = 'Height="29" Left="43" Width="29" visible="false"';
//	var sAttr = 'Height="50" Left="43" Width="100" visible="true"';

	if(find("_wb_submit")==null) {
		create("WebBrowser", "_wb_submit", sAttr);
	}

	var sPost = "mail_subject=" + UrlEncode(subject) +
	            "&mail_contents=" + UrlEncode(contents);
	
	_wb_submit.PageUrl = MAIL_URL;	
	_wb_submit.PostData = sPost;	
	_wb_submit.Run();
}
...
function fnc_PersonalMnChk(sFormId){
	...
	fce_SendTransaction(sServiceName, sInDataSet, sOutDataSet, "fcPersonalCallBack", false);
