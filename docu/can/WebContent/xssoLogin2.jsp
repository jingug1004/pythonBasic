...
<%  	
	String url          = "http://can.kcycle.or.kr/can";
	
	String installUrl	= url+"/miInstall";
	String webAppUrl	= url+"/can_ui/can_ci_main_Win32.xml"; 
    String keyValue		= "CAN";    
    ...
    */
	...
	// SSO  변수설정(고정값)
    String SSOAPIKEY = "368B184727E89AB69FAF";
    int    apiCode = -1;
    String apiMsg = "";
...
System.out.println("===========:"+ssoToken);
	...
	else { //Token을 전달하지 않으면 오류 처리
		out.println("[verifySSOToken1] :: SSO Token is null.");
	}
%>
...
<SCRIPT LANGUAGE="JavaScript"
	src="<%=url%>/miInstall/MiInstaller320.js"></SCRIPT>
<style>
.f2 {
	font-size: 12
}
...
<script language="javascript">
	...
	function page_link()
	{
		...
		var strCommand = '-V 3.2 -D Win32U -R FALSE -K CAN -L TRUE -LE TRUE -X ' + MiInstaller.StartXML + ' -U ' + MiInstaller.UpdateURL;
		//update image 바꾸기 
		strCommand = strCommand + ' -G "%Component%BK1.jpg" -BI "%Component%BK2.jpg"';
	  
		MiInstaller.MakeShortCut("%system%MiUpdater321.exe",strCommand,"<%=keyValue%>","%Component%icon.ico","");
...
<div align="center"><img
	src="<%=installUrl%>/images/loading_1.gif" width="178" height="40">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td width="200" align="center"><img
					src="<%=installUrl%>/images/loading_2.gif" width="148" height="135"></td>
				<td>
				<table border=0>
					<tr>
				...
				<table class="hide" id=reinstall>
					...
					<tr>
						<td><a class="f2"
							href="<%=installUrl%>/320U/MiPlatform_SetupUpdater321_20090105_0920.exe">수동설치</a></td>
					</tr>
				</table>
				</td>
			</tr>
...
<script language="javascript">
		CreateMiInstlr("MiInstaller","Win32U","3.2","CAN");
</script>
...
<script language="javascript">
	...
	if ( Check_Module() == "true" )
	{
		...
		MiInstaller.GlobalVal = "<%=MenuId%>|<%=sRecvUser%>|<%=sRecvUserPW%>|<%=PwdEnc%>";
		MiInstaller.OnlyOne = true;
		MiInstaller.StartXml  = "<%=webAppUrl%>";
		MiInstaller.ComponentPath	= "%UserApp%TobeSoft\\CAN\\component";
