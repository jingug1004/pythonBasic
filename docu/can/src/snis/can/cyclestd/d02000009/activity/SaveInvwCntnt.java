/*================================================================================
 * 시스템			: 후보생 면담사항 관리
 * 소스파일 이름	: snis.can.system.d02000009.activity.SaveInvwCntnt.java
 * 파일설명		: 후보생 면담사항 관리
 * 작성자			: 노인수
 * 버전			: 1.0.0
 * 생성일자		: 2008-02-27
    ...
    */
    ...
    protected int insertSaveInvwCntnt(PosRecord record,PosRecord recHead) 
    {
        ...
        int dmlcount = 0;      
			...
			param.setValueParamter(i++, Util.trim(record.getAttribute("INVW_CNTNT")));
			param.setValueParamter(i++, Util.trim(record.getAttribute("INVW_MAN")));			
			param.setValueParamter(i++, SESSION_USER_ID);	
			param.setValueParamter(i++, Util.trim(recHead.getAttribute("RACER_PERIO_NO")));
			param.setValueParamter(i++, Util.trim(recHead.getAttribute("CAND_NO")));
			
			dmlcount += this.getDao("candao").insert("d02000009_ib006", param);
    ...
    protected int updateSaveInvwCntnt(PosRecord record)
    {
        ...
        int dmlcount = 0;
			...
			dmlcount += this.getDao("candao").update("d02000009_ub006", param);    
    ...
    protected int deleteSaveInvwCntnt(PosRecord record) 
    {
        ...
        int dmlcount  = this.getDao("candao").update("d02000009_db006", param);
