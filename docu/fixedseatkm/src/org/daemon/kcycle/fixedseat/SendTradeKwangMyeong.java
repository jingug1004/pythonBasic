...
public class SendTradeKwangMyeong {    
	...
	private static ConfigFactory cf = ConfigFactory.getInstance();
	 ...
	 public void execute() {
		...
		try {
			...
			commSqlServer = conSqlServerManger.getConnection("fixedseatkm");
			...
			while (rs.next()) {
				...
				while (rs.next()) {
	 			...
	 			Log.debug("DB", this, "(광명)장소:"+CommNo+", 처리건수:"+count_src+"건");
			}
            connOracle.commit();
             
			long endTime = System.currentTimeMillis();    		 
			Log.debug("DB", this, "(광명)총 처리건수:"+count_src_tot+"건, 소요시간:"+(endTime - startTime)/1000.000f+"초");
            Log.debug("DB", this, "(광명)자료 전송 종료------------------------------------>");			 
					
		} catch (ClassNotFoundException e) {
			Log.error("ERROR", this, "(광명)JDBC 드라이버 로드가 실패하였습니다.");
		} catch (Exception e) {
			Log.error("ERROR", this, "(광명)execute : " + "Exception  [" + e.toString()+ "] ");
		} finally {
			if (rs != null) {
	...
	private int batchInsertTrade(Connection conn, ArrayList <TradeKm> arrTrade) {
		try {
			...
			Log.error("ERROR", this, "(광명)batchInsertTrade : " + "SQLException  [" + e.toString()+ "] ");
			Log.error("ERROR", this, "(광명)SQL String = " + sb.toString());
		}
		return 0;
	}
	...
	{	
		...
		try {
			...
			Log.error("ERROR", this, "(광명)UpdateMaxSeq : " + "SQLException  [" + e.toString()+ "] ");
			Log.error("ERROR", this, "(광명)SQL String = " + sb.toString());
			return -1;
		}
		return 0;
