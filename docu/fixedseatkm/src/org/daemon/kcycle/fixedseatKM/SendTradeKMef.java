...
public class SendTradeKMef {    
	...
	private static ConfigFactory cf = ConfigFactory.getInstance();
	 ...
	 public void execute() {
        Log.debug("DB", this, "(광명입장)자료 전송 시작------------------------------------>");
		try {
			...
			sb.setLength(0);
			sb.append("\n SELECT LOC, NVL(MAX(SEQ),0) AS LAST_SEQ ");
			sb.append("\n FROM   TBRC_FS_STATUS_KM   ");
			sb.append("\n WHERE  JOB_TYPE = 'E'      ");	// 광명경륜장 입장티켓			
			sb.append("\n GROUP BY LOC           ");
			...
			while (rs.next()) {
				MaxSeq = "";
				CommNo = rs.getString("LOC");				 
				MaxSeq = rs.getString("LAST_SEQ");				 
