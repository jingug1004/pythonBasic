...
<%
    String installUrl		= "http://mbr.kboat.or.kr/miInstall";
    String webAppUrl		= "http://mbr.kboat.or.kr/boa_ci_main_Win32.xml";
    String keyValue			= "MBR";
	...
	if(null == lvToken){
		lvToken = "";
		System.out.println(lvToken);	
	}
	
    if(lvToken !=null && lvToken.length() > 127  ){
    	sRecvUser    = bean.GetID(lvToken,strUserIp); //아이디;
    	sRecvUserPW  = bean.GetPWD(lvToken,strUserIp); 
    }else{
    	sRecvUser    = "";
    	sRecvUserPW  = ""; 
    }
...
<script language="javascript">
	...
	function page_link()
	{
		...
		MiInstaller.MakeShortCut("%system%MiUpdater320.exe",strCommand,"<%=keyValue%>","%Component%icon.ico","");
...
<table width="540" height="180" border="0" align="center" cellpadding="0" cellspacing="0" style="border:1px solid #dadada" >
  <tr> 
...
<script language="javascript">
	...
	if ( Check_Module() == "true" )
	{
		...
		MiInstaller.GlobalVal = "<%=MenuId%>|<%=sRecvUser%>|<%=sRecvUserPW%>";
