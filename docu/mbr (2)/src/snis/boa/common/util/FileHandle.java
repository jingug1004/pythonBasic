...
public class FileHandle {
    ...
    public static String SDL_FILE_PATH
	= System.getProperty("os.name").toUpperCase().indexOf("WIN") != -1 ? 
				"C:" + FILE_SEPERATOR + "upload_file" + FILE_SEPERATOR + "sdl" + FILE_SEPERATOR
				: "/WAS/upload_file/sdl/";
    ...
    private static String FAIR_FILE_PATH      
	= System.getProperty("os.name").toUpperCase().indexOf("WIN") != -1 ? 
				"C:" + FILE_SEPERATOR + "upload_file" + FILE_SEPERATOR + "image"  + FILE_SEPERATOR
				: "/WAS/web6/images/fairness/";
  //2014-01-09 프로펠러 이미지 경로. IWORK_SFR-012
    private static String PROP_FILE_PATH      
	= System.getProperty("os.name").toUpperCase().indexOf("WIN") != -1 ? 
				"C:" + FILE_SEPERATOR + "upload_file" + FILE_SEPERATOR + "image"  + FILE_SEPERATOR + "propeller"  + FILE_SEPERATOR
				: "/WAS/web6/images/propeller/";
...
//    			= {".jpg", ".gif", ".bmp"};
    ...
    private static String RACER_HELMET_FILE_PATH      
	= System.getProperty("os.name").toUpperCase().indexOf("WIN") != -1 ? 
				"C:" + FILE_SEPERATOR + "upload_file" + FILE_SEPERATOR + "image"  + FILE_SEPERATOR
				: "/WAS/web6/images/helmet/";
    ...
    public void upload(HttpServletRequest request, HttpServletResponse response) throws Exception {        
    	...
    	createDir(SDL_FILE_PATH);
    	createDir(MNG_IMG_FILE_PATH);
    	createDir(RACER_FILE_PATH);
    	createDir(FAIR_FILE_PATH);
    	createDir(PROP_FILE_PATH);//2014-01-09 프로펠러 이미지 경로. IWORK_SFR-012
        ...
        try{
            ...
            // 다른서버에  rcp Start
            String sServerNm = request.getServerName();
            Runtime rt = Runtime.getRuntime();

            // NAS를 사용하므로 RCP할 필요 없음(신재선 막음)
            //if (sServerNm.equals("160.100.1.52"))	rt.exec("rcp " + sWorkDirectory + sPhysicalFileName + " mrawas2:" + sWorkDirectory);
            //else									rt.exec("rcp " + sWorkDirectory + sPhysicalFileName + " mrawas1:" + sWorkDirectory);
            // 다른서버에  rcp End
            log.logDebug("request.getServerName :[" + request.getServerName() + "]");
            
            sReturn = sReturn + "physicalFileName^" + sWorkDirectory + sPhysicalFileName + "&" 
            				  + "physicalFileSize^" + nPhysicalFileSize;
            //sReturn = sReturn + "&request.getServerName^" + request.getServerName(); 
    ...
    private String getWorkDirectory(String sWork) {
    	...
    	} else if ( sWork.equals("SDL") ) {
    		sDirectory = SDL_FILE_PATH;
    	} else if ( sWork.equals("REFEREE") ) {
    		sDirectory = MNG_IMG_FILE_PATH;
    	} else if ( sWork.equals("FAIR") ) {
    		sDirectory = FAIR_FILE_PATH;
    	} else if ( sWork.equals("PROP") ) { //2014-01-09 프로펠러 이미지 경로. IWORK_SFR-012
    		sDirectory = PROP_FILE_PATH;
    	} else if ( sWork.equals("RACER_HELMET") ) { //2014-01-09 프로펠러 이미지 경로. IWORK_SFR-012
    		sDirectory = RACER_HELMET_FILE_PATH;
    	} else {
