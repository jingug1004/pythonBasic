/*================================================================================
 * 시스템			: 회차별 매출건수
 * 소스파일 이름	: snis.rbm.business.rsm3061.activity.SearchTMS.java
 * 파일설명		: 회차별 매출건수
 * 작성자			: 신재선
 * 버전			: 1.0.0
 * 생성일자		: 2014-06-13
 ...
 * 최종수정내용	: 
=================================================================================*/
package snis.rbm.business.rsm3061.activity;
...
public class SearchTMS extends SnisActivity {
	...
	private void searchTms(PosContext ctx) throws Exception {
		...
		String GREEN_YN = Util.getCtxStr(ctx,"GREEN_YN"); // 그린카드 유무
		...
		sbQuery.append(Util.getQuery(ctx, "rsm3061_select01"));
		...
		param.setWhereClauseParameter(nIdx++, RACE_DAY_ST); // 검색 시작 일
		param.setWhereClauseParameter(nIdx++, GREEN_YN); // 그린카드 포함여부
				
		if ("00".equals(COMM_NO)) // 전체 선택일 경우
		{	
			sbQuery.append("\n");
			sbQuery.append(Util.getQuery(ctx, "rsm3061_from02"));
		} 
		else 
		{
			sbQuery.append("\n");
			sbQuery.append(Util.getQuery(ctx, "rsm3061_from01"));
			
			if ("01".equals(COMM_NO)) { // 광명/미사리 본장
				param.setWhereClauseParameter(nIdx++, MEET_CD); 
				sbQuery.append(Util.getQuery(ctx, "rsm3061_where01"));
			} else if ("29".equals(COMM_NO)) { //  교차
				sbQuery.append(Util.getQuery(ctx, "rsm3061_where02"));
			} else if ("98".equals(COMM_NO) || "99".equals(COMM_NO)) {	// 미사리/광명 공동활용
				param.setWhereClauseParameter(nIdx++, MEET_CD);			 // 경륜 경정
				sbQuery.append(Util.getQuery(ctx, "rsm3061_where04"));	
			} else { // 지점
				param.setWhereClauseParameter(nIdx++, COMM_NO); // 지점 코드
				sbQuery.append(Util.getQuery(ctx, "rsm3061_where03"));
			}
		}

		sbQuery.append("\n");

		sbQuery.append(Util.getQuery(ctx, "rsm3061_groupby01"));
