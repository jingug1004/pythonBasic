 ...
 * =================================================================================
 */
package snis.rbm.business.rbo4020.activity;
...
public class SavePartsIn extends SnisActivity {
	public SavePartsIn() {
	...
	protected void saveState(PosContext ctx)
	{
		...
		if( ctx.get(sDsName) != null )
		{
			...
			for ( int i = 0; i < nSize; i++ )
			{
				...
				{
					nTempCnt = insertPartsIn(record);		// 입고 등록
							   UpdatePartsStock(record);	// 현재고 수정
				...
				{
					nDeleteCount = nDeleteCount + deletePartIn(record);			// 입고삭제
												  DeletePartsStock(record);		// 현재고 수정
				}		
			}
		}
	...
	protected int insertPartsIn(PosRecord record)
	{
		...
		int dmlcount = this.getDao("rbmdao").update("rbo4020_i01", param);
	...
	protected int deletePartIn(PosRecord record)
	{
		...
		param.setValueParamter(i++, SESSION_USER_ID					 );	// 1. 부품코드
		param.setValueParamter(i++, record.getAttribute("PARTS_CD"	));	// 2. 부품코드
		param.setValueParamter(i++, record.getAttribute("IN_DT"		));	// 3. 입고일자
		
		int dmlcount = this.getDao("rbmdao").update("rbo4020_d01", param);
		return dmlcount;
