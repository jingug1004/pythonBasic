...
public class SearchDIVTMSCNT extends SnisActivity {
	...
	private void searchDivTmsCnt(PosContext ctx) throws Exception {
		...
		sbQuery.append(Util.getQuery(ctx, "rsm3060_select01"));

		// 기준년도 값 이런식으로 값 안넣으면 멈춤 
		sbQuery.append(" \n");
		sbQuery.append(" AND B.STND_YEAR = SUBSTR( '"+RACE_DAY_ST+"' ,1,4)	-- 4:RACE_DAY_ST 기준년도  \n");
		sbQuery.append(" AND A.STND_YEAR = SUBSTR( '"+RACE_DAY_ST+"' ,1,4)	-- 5:RACE_DAY_ST 검색 기간 시작 일 \n");
		
		sbQuery.append("\n");
		sbQuery.append(Util.getQuery(ctx, "rsm3060_groupby01"));
		
		prs = this.getDao("rbmdao").findByQueryStatement(sbQuery.toString(),param);
		String sOutDs = "dsDIVTMSCNT";
		ctx.put(sOutDs, prs);
