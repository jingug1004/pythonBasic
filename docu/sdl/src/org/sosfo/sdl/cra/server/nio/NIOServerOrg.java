 ...
 * </pre>
 */
public class NIOServerOrg extends Thread {
	...
	NIOServerOrg nioServer = new NIOServerOrg();
	nioServer.start();
    }
	...
	try {
		...
		CharBuffer charBuffer = charset.decode(sharedBuffer);
		conts = charBuffer.toString();
		Log.info("NIO", conts);

		String returnMsg = "Good job! Thank you";

		sharedBuffer = charset.encode(returnMsg);
		...
		// whether Broadcast SDL or not
		if (broadcast) {
		    BroadMultiCast cast = new BroadMultiCast(conts);
		    cast.start();
		}
	    }
	} catch (IOException e) {
