 ...
 * 최종수정내용	: 
=================================================================================*/
package snis.can_boa.boatstd.d06000004.activity;
...
public class SaveWeekEduSchd extends SnisActivity 
{
     ...
     */
     ...
     protected int insertWk_Sche(PosRecord record) 
     {
         ...
         logger.logInfo("YY ==>"+Util.trim(record.getAttribute("YY")));
         logger.logInfo("MM ==>"+Util.trim(record.getAttribute("MM")));
         //logger.logInfo("WK ==>"+record.getAttribute("WK"));
         logger.logInfo("WK ==>"+Util.trim(record.getAttribute("WK")));
         ...
         String rPerioNo = Util.trim(record.getAttribute("RACER_PERIO_NO"));
         
         param.setValueParamter(i++, rPerioNo);
         ...
         param.setValueParamter(i++, Util.trim(record.getAttribute("WK")));
         ...
         param.setValueParamter(i++, rPerioNo);	 
 		 ...
 		 int dmlcount = this.getDao("candao").insert("tbdn_wk_sche_in001", param);
     ...
     protected int updateWk_Sche(PosRecord record) 
     {
		...
		int dmlcount = this.getDao("candao").update("tbdn_wk_sche_un001", param);
     ...
     protected int deleteWk_Sche(PosRecord record) 
     {
         ...
         int dmlcount = this.getDao("candao").delete("tbdn_wk_sche_dn001", param);
