package snis.can_boa.system.d05000007.activity;
...
import snis.can_boa.common.util.UtilDb;
...
public class SaveMenuAuth extends SnisActivity {
    ...
    */
    ...
    protected int updateAuthMenu(String MenuID, String curDtm, PosRecord record) 
    {
 		...
 		param.setWhereClauseParameter(i++, sStrDtTm);		// 현재 시점의 권한 여부를 체크
 		
 		rowset = this.getDao("candao").find("d05000007_s04", param);
 		...
 		if (bAuthExist == false && bAuthSet == true) {
	        ...
	        param.setValueParamter(i++, sSrchYn);
 	        ...
 	        dmlcount += this.getDao("candao").update("d05000007_i01", param);
 	        ...
 	        dPercount += this.getDao("candao").update("d05000007_i02", perparam);
 	    // 3)자료가 있으면
	        ...
	        dmlcount += this.getDao("candao").update("d05000007_u01", param);
	        ...
	        perparam.setValueParamter(i++, sPerMnIp); 
 	        ...
 	        dPercount += this.getDao("candao").update("d05000007_i03", perparam);
 		}		
