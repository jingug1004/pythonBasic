...
function fnc_FormLoadSetting(obj, sFORMCAPTION, sFORMLOCATION, sBUTTONLIST, sUSERAUTH) {
	...
	//obj.stFormTitle     = sFORMCAPTION+"("+obj.stFormId.Value+")";          //화면 정보 Display
	if (isExistVar("sFormFileNm")) {
		obj.stFormTitle     = sFORMCAPTION+"("+obj.sFormFileNm+")";          //화면 정보 Display
	} else { 
		obj.stFormTitle     = sFORMCAPTION+"("+obj.sFormId+")";          //화면 정보 Display
	}
...
+-------------------------------------------------------------------------------------------------*/
function fnc_Message(sMessageID, sMessageType, sArg1, sArg2, sArg3, sArg4, sArg5, sBtnType){
...
function IsColumnDuplicate(oDataSet, colID) {
   
    var cnt = 0;
    if ( (oDataSet.rowcount >0) && (!fnc_IsNull(colID)) )
    {
        alert(oDataSet.rowcount+"::::"+colID);
        // 시간배정표 과목 중복입력 체크
        for(var i=0; i<oDataSet.rowcount; i++) {
            
            var originVal = oDataSet.GetColumn(i, colID); 
            for(var j=1; j<oDataSet.rowcount; j++) {
                
                var compareVal = oDataSet.GetColumn(j, colID);
                if(originVal == compareVal) {
...
function fnc_ReportPreView(rptFile, paramDsId, sRptDrct, fullSize)
{
	...
	open(sFileName, sArg, nWidth, nHeight, "AutoSize=false Resize=true", nPosX, nPosY);
...
function fnc_SendMessage(sendUser, recvUser, title, linkUrl, desc, compId, systemId) {
    ...
    if (systemId == "BOA") { //경정운영련관리
		ServerURL = "http://mbr.kboat.or.kr/miInstall/xssoLogin.jsp?MENUID=";
    }
   ...
   for(var i = 0; i < aRecvId.length;i++){
		...
		//개발서버 OR 로컬서버
        obj.Navigate(urlEncodeUtf8(GBL_SERVER_URL+"send.jsp?SENDUSER="+sendUser+"&RECVUSER="+aRecvId[i]+"&TITLE="+title+"&LINKURL="+tmpServerURL+"&DESC="+desc));
		//Trace(urlEncodeUtf8(GBL_SERVER_URL+"send.jsp?SENDUSER="+sendUser+"&RECVUSER="+aRecvId[i]+"&TITLE="+title+"&LINKURL="+tmpServerURL+"&DESC="+desc));
   }   
}
