					...
					<table>
						...
						<tbody>
							...
							<tr>
								<%if(!"E02004".equals(state)){%>
								<th>조퇴(발생시각)</th>
								<td class="bdrn" colspan="2"><%=StringUtil.nvl(commuteVO.getLeave_tm())%></td>
								<%}else{ %>	
								<th>조퇴(발생시각)</th>
								<td class="bdrn"><%=StringUtil.nvl(commuteVO.getLeave_tm())%></td>
								<%}%>									
							</tr>
							<tr>
								<%if(!"E02004".equals(state)){%>
							...
							<tr>
								<%if(!"E02004".equals(state)){%>
								<th>내용</th>
								<td class="bdrn" colspan="2"><%=StringUtil.nl2br(RequestFilterUtil.toHtmlTagReplace("", commuteVO.getReason()))%></td>
								<%}else{ %>
								<th>내용</th>
								<td class="bdrn"><%=StringUtil.nl2br(RequestFilterUtil.toHtmlTagReplace("", commuteVO.getReason()))%></td>
								<%}%>
							</tr>
						</tbody>
					</table>
