...
<%--
 * @파일명 : messageReplyWrite.jsp
 * @메뉴명 : 쪽지 답장 쓰기
 * @최초작성일 : 2015/06/10            
 * @author : Yun.Beom.Jin(kiba15@irush.co.kr)
 * @수정내역 : 
--%>
...
<%
	MessageVO messageDetail = (MessageVO)request.getAttribute("messageDetail"); // 원본 쪽지 정보
	MemberVO senderDetail = (MemberVO)request.getAttribute("senderDetail"); // 발신자 정보
	
	String dept_nm = senderDetail.getDept_ko_nm();
	String grad_nm = senderDetail.getGrad_ko_nm();
	String emp_nm = senderDetail.getEmp_ko_nm();
	
	String subject = "RE : " + messageDetail.getSubject();
	String contents = "<br/><br/><br/>---------- Original Message ----------<br/>";
	contents += "From : " + dept_nm + " " + grad_nm + " " + emp_nm + "<br/>";
	contents += "Date : " + messageDetail.getCreate_dt() + "<br/>";
	contents += "Subject : " + messageDetail.getSubject() + "<br/><br/>";
	contents += messageDetail.getContents();
%>
<!DOCTYPE html>
<html lang="ko">
<head>
	...
	function getMember(message_seq){
		$('#getMember').bPopup({
            ...
            opacity: 0.6,
            positionStyle: 'fixed',
			loadUrl:'<%=GROUP_ROOT %>/pe/member/memberSelectPopup.do?type=MESSAGE_REPLY&seq='+message_seq,
		});
	}
			...
			<div class="cont float_left">
				...
				<div class="noticeboard_box">
					...
					<div class="cont_box cont_table06">
						<table>
							<colgroup>
								...
								<tbody>
									<tr>
										<th class="bt_none bc_re br_none" colspan="2">답장쓰기</th>
									</tr>
									<tr>
										<th class="">제목</th>
					...
					<div class="append_object_box">
						<!-- <h3>받는이</h3>  -->
						<button class="type_01" type="button" onclick="javascript:getMember('<%=messageDetail.getMessage_seq()%>');">받는이</button>						
							...
							<table id="tbl">
								...
								<tbody>
									<tr>
										...
										<td class="br_none">
											<%=emp_nm %>
											<input type="hidden" id="target_empNos" name="target_empNos" value="<%=senderDetail.getEmp_no() %>" />
										</td>
									</tr>
								</tbody>
							</table>
