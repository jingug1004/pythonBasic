...
<%--
 * @파일명 : yearendtaxHousingFundPopup.jsp
 * @메뉴명 : 연말정산 등록 > 주택계약 팝업
 * @최초작성일 : 2015/02/16
 * @author : 윤범진                  
 * @수정내역 : 
...
<head>
	...
	<script type="text/javascript">
		...
		function addRow(){
			...
			html += '			<option value="2" selected>주택임차차입금 금전소비대</option>';
			html += '			<option value="3" >주택임차차입금 임대차</option>';
			html += '		</select>';
			html += '	</td>';
															...
															<table>
																...
																<%
																	for(int i = 0; i < houseList.size(); i++){
																		...
																		String house_amt3 = StringUtil.makeMoneyType(houseDetail.getHouse_amt3());
																		String house_amt4 = StringUtil.makeMoneyType(houseDetail.getHouse_amt4());
																		String house_amt5 = StringUtil.makeMoneyType(houseDetail.getHouse_amt5());
																	...
																	<tr id="houseList_<%=i+1 %>">
																		...
																		<td>
																			<select name="house_gb" id="house_gb_<%=i+1 %>">
																				<option value="2" <% if("2".equals(house_gb)) { %>selected<% } %>>주택임차차입금 금전소비대</option>
																				<option value="3" <% if("3".equals(house_gb)) { %>selected<% } %>>주택임차차입금 임대차</option>
																			</select>
																		</td>
																		...
																		<td class="date">
																			<span class="date_box">
																				<input type="text" class="serch_date" name="house_start_dt" id="house_start_dt_<%=i+1 %>" value="<%=house_start_dt %>" class="" />
