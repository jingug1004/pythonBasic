...
<head>
	...
	$(document).ready(function(){
		...
		$("#grid_list").jqGrid({
			...
			colNames:["파트코드","파트명"], // 컬럼명
			colModel:[
						{name:"part_id",	index:"part_id", 	align:"center"},
						{name:"part_nm",	index:"part_nm", 	align:"left"},
					],
			height:391,
			rownumWidth : 30,
			page: 1,
			...
			ondblClickRow: function(){
				Business.selectValue('part'); // 팝업을 닫고 부모창에 선택한 값 셋팅
			},
	...
	function getGridList(){
		...
		/* 호출 */
		$("#grid_list").jqGrid('setGridParam',{url:"<%=ONLINE_ROOT%>/business/common/partGridList.do?" + param}).trigger("reloadGrid");
	}
...
<body>
	<div class="popup" title="Main">
		<h1 class="tit">파트조회</h1>
		
		<div class="wrap_pop_search">
			<label>파트코드/명</label>
			<input type="text" class="w340" maxlength="50" id="searchKeyword" name="searchKeyword" />
			<button type="button" onclick="javascript:getGridList();" >조회</button>
			<button type="button" onclick="javascript:Business.selectValue('part');" >선택</button>
		</div>
