...
<%@ include file ="/common/path.jsp" %>
	...
	function addRow() {
		...
		objCel1.setAttribute("class", "name");
	   	...
	   	objCel1.setAttribute("class", "time");
	   	objRow.appendChild(objCel1);
	   	objCel1.innerHTML="<input type='text' name='work_due_hh' id='work_due_hh"+cnt+"' class='ipt_time' onchange='workRealTime("+cnt+");' readonly/> : <input type='text' name='work_due_mi' id='work_due_mi"+cnt+"' class='ipt_time' onchange='workRealTime("+cnt+");' readonly />";
	...
	function saveStep1(){
		...
		var work_due_hh = document.getElementsByName("work_due_hh");
		var work_due_mi = document.getElementsByName("work_due_mi");
		...
		for(var i=0; i<work_due_ymd.length; i++){
			...
			if(formCheck.isNull(emp_nm[i], "성명을 입력해 주세요.")){
				return;
			}
			...
			if(formCheck.isNull(work_due_ymd[i], "근무예정일을 입력해 주세요.")){
				return;
			}
			
			if(formCheck.isNull(work_start_hh[i], "근무시작시간을 입력해 주세요.")){
				return;
			}
			...
			if(formCheck.isNull(work_start_mi[i], "근무시작시간을 입력해 주세요.")){
				return;
			}
			...
			if(formCheck.isNull(work_end_hh[i], "근무종료시간을 입력해 주세요.")){
				return;
			}
			...
			if(formCheck.isNull(work_end_mi[i], "근무종료시간을 입력해 주세요.")){
				return;
			}
			...
			//console.log("i : "+i);
			//workRealTime(i);
			if(formCheck.isNull(work_due_hh[i], "실근무시간을 입력해 주세요.")){
				return;
			}
			
			if(formCheck.isNull(work_due_mi[i], "실근무시간을 입력해 주세요.")){
				return;
			}
		}
	...
	function workRealTime(target){
		...
		if(work_end_hh.value != null && work_end_hh.value !="" && work_end_mi.value != null && work_end_mi.value !=""){
			...
			if(work_start_value >= work_end_value){
				...
				document.getElementById("work_due_hh"+target).value = "";
				document.getElementById("work_due_mi"+target).value = "";
				document.getElementById("work_start_hh"+target).focus();
				return;
			}
			
			var work_due_mi_value = work_end_value - work_start_value;
			var hours = parseInt(work_due_mi_value / 60 );
			var minutes = parseInt(work_due_mi_value % 60 );
	
			if( 12 >= work_start_hh_value && 13 <= work_end_hh_value){//12-13시를 포함할 경우 점심시간 1시간 마이너스 한다.
				document.getElementById("work_due_hh"+target).value = formCheck.leadingZeros(hours-1, 2);
				document.getElementById("work_due_mi"+target).value = formCheck.leadingZeros(minutes, 2);
			}else{
				document.getElementById("work_due_hh"+target).value = formCheck.leadingZeros(hours, 2);
				document.getElementById("work_due_mi"+target).value = formCheck.leadingZeros(minutes, 2);
			}
		}
	}	
							...
							<div class="inner_box no_scroll">
								...
								<table class="tbl_overtime" id="tbl">
									...
									<tbody>
										...
										%>
										<tr>
											<td onclick="javascript:memberSearchPopup('<%=i%>');"><textarea name="dept_nm" id="dept_nm<%=i%>" class="ta_overtime" readonly><%=StringUtil.nvl(overtimeVO.getDept_nm()) %></textarea><input type="hidden" name="dept_cd" id="dept_cd<%=i%>" value="<%=StringUtil.nvl(overtimeVO.getDept_cd()) %>" ></td>
											<td class="name" onclick="javascript:memberSearchPopup('<%=i%>');"><input type="text" name="emp_nm" id="emp_nm<%=i%>" value="<%=StringUtil.nvl(overtimeVO.getEmp_nm()) %>" class="ipt_overtime" readonly/><input type="hidden" name="emp_no" id="emp_no<%=i%>" value="<%=StringUtil.nvl(overtimeVO.getEmp_no()) %>" ><button type="button" onclick="javascript:memberSearchPopup('<%=i%>');">조회</button></td>
											<td>
											...
											<td class="date">
												<span class="date_box">
													<input type="text" class="serch_date" name="work_due_ymd" value="<%=StringUtil.nvl(overtimeVO.getWork_due_ymd()) %>" readonly/>
													<button type="button" class="btn_date"><span class="blind">날짜선택</span></button>
												</span>
											</td>
											<td class="time"><input type="text" name="work_start_hh" id="work_start_hh<%=i%>" value="<%=StringUtil.nvl(overtimeVO.getWork_start_hh()) %>" class="ipt_time" onchange="workRealTime(<%=i%>);"/> : <input type="text" name="work_start_mi" id="work_start_mi<%=i%>" value="<%=StringUtil.nvl(overtimeVO.getWork_start_mi()) %>" class="ipt_time" onchange="workRealTime(<%=i%>);"/></td>
											<td class="time"><input type="text" name="work_end_hh" id="work_end_hh<%=i%>" value="<%=StringUtil.nvl(overtimeVO.getWork_end_hh()) %>" class="ipt_time" onchange="workRealTime(<%=i%>);"/> : <input type="text" name="work_end_mi" id="work_end_mi<%=i%>" value="<%=StringUtil.nvl(overtimeVO.getWork_end_mi()) %>" class="ipt_time" onchange="workRealTime(<%=i%>);"/></td>
											<td class="time"><input type="text" name="work_due_hh" id="work_due_hh<%=i%>" value="<%=StringUtil.nvl(overtimeVO.getWork_due_hh()) %>"  class="ipt_time" readonly /> : <input type="text" name="work_due_mi" id="work_due_mi<%=i%>" value="<%=StringUtil.nvl(overtimeVO.getWork_due_mi()) %>"  class="ipt_time" readonly/></td>
										...
										%>
										<tr>
											<td onclick="javascript:memberSearchPopup('0');"><textarea name="dept_nm" id="dept_nm0" class="ta_overtime" readonly></textarea><input type="hidden" name="dept_cd" id="dept_cd0" value=""></td>
											<td class="name" onclick="javascript:memberSearchPopup('0');"><input type="text" name="emp_nm" id="emp_nm0" value="" class="ipt_overtime" readonly/><input type="hidden" name="emp_no" id="emp_no0" value=""><button type="button" onclick="javascript:memberSearchPopup('0');">조회</button></td>
											<td>
												<select name="bigo" id="bigo" class="sel_certificate">
													<option value="">근무구분 선택</option>
												<%
											...
											<td><textarea name="biz_content" class="ta_overtime"></textarea></td>
											<td class="date">
												<span class="date_box">
													<input type="text" class="serch_date" name="work_due_ymd" readonly/>
													<button type="button" class="btn_date"><span class="blind">날짜선택</span></button>
												</span>
											</td>
											<td class="time"><input type="text" name="work_start_hh" id="work_start_hh0" class="ipt_time" onchange="workRealTime('0');"/> : <input type="text" name="work_start_mi" id="work_start_mi0" class="ipt_time" onchange="workRealTime('0');"/></td>
											<td class="time"><input type="text" name="work_end_hh" id="work_end_hh0" class="ipt_time" onchange="workRealTime('0');"/> : <input type="text" name="work_end_mi" id="work_end_mi0" class="ipt_time" onchange="workRealTime('0');"/></td>
											<td class="time"><input type="text" name="work_due_hh" id="work_due_hh0" value="" class="ipt_time" readonly /> : <input type="text" name="work_due_mi" id="work_due_mi0" value="" class="ipt_time" readonly/></td>
											<td class="bdrn btn"><button type="button" onClick='removeRow(this);'>삭제</button></td>
										</tr>
										<%} %>
									</tbody>
