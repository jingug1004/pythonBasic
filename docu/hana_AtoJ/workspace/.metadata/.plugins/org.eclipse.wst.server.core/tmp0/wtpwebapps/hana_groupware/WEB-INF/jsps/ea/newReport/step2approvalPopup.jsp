...
<%
	...
	//----------------------------------------------------------------------------------------------
	//CHOE 20171023 관리부 이준 요청 관리부0013 를 시행부서로 넣는 경우 법무팀0527을 시행 부서로 넣어 준다 : 문서유형의 구분 지금은 없음
	String CheckDocuManage = "Y";  
	...
	function goStep3(){
		...
		if(subtmit == false){
			if(confirm("저장하시겠습니까?") == true){
				...
				if(approval_emp_no.length> 0){
					...
					for(var i=0; approval_emp_no.length> i;i++){
						...
						if(checkValueArr[1] == "INSERT"){//기존에 승인중인 라인은 그대로 두고 신규 결재자만 저장한다.
							//-----------------------------------------------------------------------------------------
							//20161005 전산팀장님과 윤홍주부장 하드코드 추가 조동훈 부사장이 결재에 포함되는 경우 시행부로 추가한다.
				...
				var addDept = "N";
				//CHOE 20171023 관리부0013가 추가된 경우 법무팀0527 추가
				var addDeptManage = "N";
				if(implDept_dept_cd.length> 0){
					implDept = new Array();
					for(var i=0; implDept_dept_cd.length> i;i++){
						...
						//----------------------------------------------------------------------------------------------
						//CHOE 20171023 관리부0013가 추가된 경우 법무팀0527 추가
						if("<%=CheckDocuManage%>" == "Y"){											
							//1-1.이미 법무팀 부서가 있는 경우 예외처리
							if (implDept_dept_cd[i].value == "0527"){  
								addDeptManage = "N";  								
							}
							if (implDept_dept_cd[i].value == "0367"){  
								addDeptFool = 'N';   //CHOE 20161005 중복입력 배제 이곳은 중복 입력 배제를 위해 꼭 넣어 준다
							}
							//1-2. 관리부를 넣은 경우 한번만 실행 한다.							
							if (addDeptManage == "N"){
								if (implDept_dept_cd[i].value == "0013"){ 
									addDeptManage = "Y";								
								} 
							}
						}
					...
					//CHOE 20171023 관리부0013가 추가된 경우 법무팀0527 추가
					if (addDeptManage == "Y"){ 
						alert("관리부를 시행부서로 넣는 경우 법무팀을 꼭 시행부서로 추가해야 합니다.");
						implDept.push('0527'+"|"+(i+1));
					}
					//20161005 전산팀장님과 윤홍주부장 하드코드 추가 조동훈 부사장이 결재에 포함되는 경우 시행부로 추가한다.
					if (addDeptFool == "Y"){
