...
<%--
 * @파일명 : personApprovalMemberTargetIframe.jsp
 * @메뉴명 : 개인결재라인 결재대상 iframe
 * @최초작성일 : 2015/01/28            
 * @author : CHOIILJI                 
 * @수정내역 : 
--%>
	...
	function addApprovalMultiRow(){
	    ...
	    for(var i=0; i<approvalListCheckBoxCnt; i++){
	    	if(approvalListCheckBoxArr[i].checked){
				...
				if(memberListCheckValueArr[0] == <%=memberSessionVO.getEmp_no()%>){
		    		alert("자신은 결재라인에 포함 할 수 없습니다.");
		    		approvalListCheckBoxArr[i].checked = false;
		    	}
		    }
	    } 
...
<body>
	<div class="settle_men fl" data-role="ui-droplist" data-type="basic">
		...
		<ul id="approval_ul" class="select_list fl" data-role="ui-droplist-list">
			...
			%>
			<li data-role="ui-droplist-item"><%=personApprovalVO.getEmp_ko_nm() %><input type="hidden" name="approval_emp_no" value="<%=personApprovalVO.getEmp_no()%>" ></li>
			<%
				}
			}
			%>		
