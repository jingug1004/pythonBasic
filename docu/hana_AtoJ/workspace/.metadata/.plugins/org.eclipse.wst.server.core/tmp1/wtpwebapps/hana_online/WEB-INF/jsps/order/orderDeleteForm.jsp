	...
	function orderApprovalAjax(){
		...
		$.ajax({
			...
			success:function(data){
				...
				if(data.ls_receipt_gb=='3' || data.ls_receipt_gb=='03'){
					alert('요청일자:'+ mGrid.getRowData(mlastSelection).ymd +'\n접수번호:' + mGrid.getRowData(mlastSelection).gumae_no + '는 주문 자료가 승인되어 삭제할수 없습니다.');
					deleteProcFlag = false;
					return;						
				/*}else if(data.ls_receipt_gb=='1' || data.ls_receipt_gb=='01' ){
					alert('요청일자:'+ mGrid.getRowData(mlastSelection).ymd +'\n접수번호:' + mGrid.getRowData(mlastSelection).gumae_no + '는 주문 자료가 상신되어 삭제할수 없습니다.');
					deleteProcFlag = false;
					return;
