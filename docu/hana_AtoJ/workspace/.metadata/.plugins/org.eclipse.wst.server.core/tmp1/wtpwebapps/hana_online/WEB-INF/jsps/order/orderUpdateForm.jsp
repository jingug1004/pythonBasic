...
<head>
	...
	$(document).ready(function(){
		...
		*/
		...
		if(StringUtil.nvl(orderInit.getLs_sawon_info(),"").equals("")){		//담당자 자료 유무 체크
			out.println("alert('"+userEmpCode+"에 대한 거래처(담당자) 자료가 없습니다.');");			//javascript alert
		}
	...
	--%>
		...
		$("#grid_list2").jqGrid({
			...
			mtype:"post",
			datatype:"json",
			colNames:["제품코드","제품명","규격","주문수량","변경수량","단가","공급가액","세액","공급총액","tot_amt","dc_danga","dc_amt","input_seq","gumae_no"],
			colModel:[
		...
		if(lastSelection == 0){
			...
			if(ld_amend_qty == '0' || ld_amend_qty == ''){
				...
				*/
				...
				var order_no = row.gumae_no;
				var ls_p_orderqty = row.qty.replace(/,/gi,""); 
				var ls_p_salprc = row.danga.replace(/,/gi,""); //주문단가
				var ls_p_modifyqty = $('#'+row.item_id+'_amend_qty').val();// 주문수량
				$.ajax({
					type:"POST",
					async: false,
					url:"<%=ONLINE_ROOT%>/order/itemChkEditAjax.do",
					data:{ls_item_cd:ls_item_cd, ldt_jaego:ldt_jaego, ls_rcust_id:ls_rcust_id ,ls_p_orderqty:ls_p_orderqty, ls_p_salprc:ls_p_salprc , ls_p_modifyqty:ls_p_modifyqty,orderno:order_no},
					dataType:"json",
					success:function(data){
						...
						if("*" == data.pieceyn){
							$('#pro').bPopup().close();
							alert('제품코드 : '+ (ls_item_cd) +'\n' + (ls_item_nm) + ' ' + (ls_standard) + '는 낱알로 등록할 수 없는 제품입니다.');
						...
						for(var test = 0 ; test < chkReturnNums.length ; test++){
							try{
								if(data.itemChk[chkReturnNums[test]] != '' && data.itemChk[chkReturnNums[test]] != null){
									$('#pro').bPopup().close();
									alert('주문삭제 처리 하세요. \n'+data.itemChk[chkReturnNums[test]]+' \n필요시 담당자에게 문의 하시기 바랍니다.');
									dGrid.setSelection(lastSelection);
									$('#'+row.item_id+'_amend_qty').val("");
									inserYn=0;
									return;
						...
						var chkReturnNums2 = ['20-1','20-2','20-2','20-4','20-5','20-6'];
						for(var test = 0 ; test < chkReturnNums.length ; test++){
							try{
								if(data.itemChk[chkReturnNums2[test]] != '' && data.itemChk[chkReturnNums2[test]] != null){
									$('#pro').bPopup().close();
									alert(data.itemChk[chkReturnNums2[test]]);
									dGrid.setSelection(lastSelection);
									$('#'+row.item_id+'_amend_qty').val("");
									dGrid.editRow(lastSelection,true);
									inserYn=0;
						...
						//총액 
						amend_tot_amt = ld_amend_qty * ld_danga;
						
						amend_vat = Math.floor(amend_tot_amt * 0.1);
						//공급가액
						amend_amt = amend_tot_amt - amend_vat;
...
*/							
						...
						if(parseInt(ld_credit_limit_amt) > 0){
							...
							if(parseInt(total_amt,10) >  (parseInt(ld_rem_dambo,10) + parseInt(tot_amt ,10) )  ){
								alert('여신한도액을 초과하였습니다. 담당자에게 문의하세요.1');
								dGrid.setSelection(lastSelection);
								dGrid.editRow(lastSelection,true);
	...
	*/
	...
	function updateOrderAjaxBeforeCheck(idx){
	
		var grid = $("#grid_list2");
		
		var gridROwCount = $("#grid_list2").getGridParam("reccount");

		if(idx == gridROwCount){
			//마지막 ROw 까지 수행 한 뒤 저장
			updateOrderAjax();
		}
		
		var row = grid.getRowData(idx);

		if(row.amend_qty == '' || row.amend_qty == null){
			//변경 한 수량이 없다면 다음 ROw 수행
			updateOrderAjaxBeforeCheck(idx + 1);
		}
		...
		var ls_p_orderqty = row.qty.replace(/,/gi,"");
		var ls_p_salprc = row.danga.replace(/,/gi,""); //주문단가
		var ls_p_modifyqty = row.amend_qty;
		var order_no = row.gumae_no;
		...
		for(var i = 0 ; i < gridROwCount ; i++){
			var mRow = mGrid.getRowData(idx);
			
			if(mRow.gumae_no == order_no){
				ls_rcust_id = mRow.rcust_id;
				break;
			}
		}
	...
	*/
	function updateOrderAjax(){
		...
		$.ajax({
			type:"POST",
			url:"<%=ONLINE_ROOT%>/order/common/storeChkAjax.do",
			data:{ rcust_id : $("#rcust_id").val() ,tot_amt : $("#tot_amt").val().replace(/,/gi,"")},
			dataType:"json",
			success:function(data){
				...
				if(itemIdArray != ''){
					...
					for(var i=0; i < arry.length; i++ ){
						...
						var danga = row.danga.replace(/,/gi,"");
						...
						$("<input></input>").attr({type:"hidden", 	name:"danga", 		value:danga}).appendTo(itemFrm);
