...
<head>
	...
	$(document).ready(function(){
		...
		$("#grid_list_part").jqGrid({
			...
			loadComplete: function(data){
				if(data.records > 0){
					...
					/* 2018-01-19 실시간으로 변경 (향후 재사용 될 수 있어서 주석 처리)
		...
		/* 조회일자 셋팅(이번 달 -> from,to 로 변경 (2018-01-19)) */
		//Commons.setDate("ad_fr_date", "ad_to_date");
		Business.setThisMonth("as_date_fr");
		Business.setThisMonth("as_date_to");
		/* grid 영역 넓이 셋팅 */
	...
	function getGridList(){
		...
		var as_date_fr = $("#as_date_fr").val();
		var as_date_to = $("#as_date_to").val();

		<%-- 2018-01-17 부가세 계산 안해도 되므로 삭제
		...
		*/
		/* 유효성 체크 */
		if (!formCheck.isDateFormat(as_date_fr+"-01")) {
			alert("조회 기간을 입력하세요!");
			$("#ad_fr_date").focus();
			return false;
		}
		
		if (!formCheck.isDateFormat(as_date_to+"-01")) {
			alert("조회 기간을 입력하세요!");
			$("#ad_to_date").focus();
			return false;
		}
		
		if (formCheck.dateChk(as_date_fr+"-01", as_date_to+"-01") < 0) {
			alert("조회 기간을 확인하세요!");
			return false;
		}
		...
		<div class="wrap_search">
			<div class="search">
				<div class="float_l">
					...
					<input type="text" class="w50" maxlength="7" id="as_date_fr" name="as_date_fr" />
					~
					<input type="text" class="w50" maxlength="7" id="as_date_to" name="as_date_to" /><br />

					<label class="w70">파 트</label>
					<input type="text" class="w120 <%=part_ipt_disable %>" id="as_part_cd" name="as_part_cd" onblur="javascript:getPerformanceName(this.id, 'part');" value="<%=part_gb %>" <%=part_readonly %>/>
					...
					<input type="text" class="w245 ipt_disable" id="as_part_cd_name" name="as_part_cd_name" value="<%=part_name %>" readonly />
				</div>
				
				<div class="float_l m110" style="margin-left: 30px;">
					<br/>
					
					<label class="w70">부 서</label>
					<input type="text" class="w120 <%=dept_ipt_disable %>" id="as_team_cd" name="as_team_cd" onblur="javascript:getPerformanceName(this.id, 'team');" value="<%=dept_cd %>" <%=dept_readonly %>/>
					...
					<input type="text" class="w245 ipt_disable" id="as_team_cd_name" name="as_team_cd_name" value="<%=dept_name %>" readonly /><br />
				
					<label class="w70">사 원</label>
					<input type="text" class="w120 <%=emp_ipt_disable %>" id="as_emp_no" name="as_emp_no" onblur="javascript:getPerformanceName(this.id, 'emp');" value="<%=emp_cd %>" <%=emp_readonly %>/>
					...
					<input type="text" class="w245 ipt_disable" id="as_emp_no_name" name="as_emp_no_name" value="<%=emp_name %>"  readonly />
