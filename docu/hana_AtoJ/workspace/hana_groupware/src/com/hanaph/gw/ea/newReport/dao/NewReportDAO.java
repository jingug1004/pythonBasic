 ...
 * @since   : 2014. 12. 29.
 */
public interface NewReportDAO {

	String insertApprovalMaster(ApprovalMasterVO paramAmVO);
	 ...
	 * @return
	 */
	boolean updateApprovalMasterComplete(ApprovalMasterVO paramAmVO);
	...
	/**
	 * <pre>
	 * 1. 개요     : 신규문서작성 상세정보를 가져온다.
	 * 2. 처리내용 : 신규문서작성 상세정보를 가져온다.
	 * </pre>
	 ...
	 * <pre>
	 * 1. 개요     : 전자결재 첨부파일 수정
	 * 2. 처리내용 : 전자결재 첨부파일 등록 후 seq파라미터값을 받아 업데이트 한다. 
	 * </pre>
	 ...
	 * <pre>
	 * 1. 개요     : 시행부서 의견 필수 유무
	 * 2. 처리내용 : 시행부서 의견 필수 유무로 결재 테이블을 변경하다.
	 * ml180118.ml03_T25 김진국 - NewReportDAO.java에 addMustOpinionApproval 인터페이스 추가 - '시의필' 버튼 누르면 결재 마스터(GW_EA_APPROVAL_MASTER) 뿐만 아니라 결재 테이블(GW_EA_APPROVAL)도 변경하기 위해서
	 * </pre>
	 ...
	 * 1. 개요     : 결재 제목
	 * 2. 처리내용 : 결재 제목을 가져온다.
	 * ml180122.ml07_T46 김진국 - newReportDAO.java에 getApprovalSubject 인터페이스 추가 - 쪽지에 결재 제목을 가져오기 위해서
	 * </pre>
	 ...
	 * ml180124.ml03_T72 김진국 - newReportDAO.java에 getApprovalerMustOpinion 추가 - 시의필 실행시 결재 작성자와 결재자에게도 쪽지 보내기 위해서!
	 * </pre>
	 * @Method Name : getApprovalSubject
	 * @return
	 */
	...
	/**
	 * <pre>
	 * 1. 개요     : 해당 결재문서의 시의필 Y -> N
	 * 2. 처리내용 : 해당 결재문서의 시의필 Y -> N 변환한다.
	 ...
	 * </pre>
	 * @Method Name : addMustOpinionYtoN
	 * @return
	 */
