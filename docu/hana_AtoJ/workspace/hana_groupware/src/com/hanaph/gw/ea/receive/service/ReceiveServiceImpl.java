...
public class ReceiveServiceImpl implements ReceiveService {
	...
	/* (non-Javadoc)
	 * @see com.hanaph.gw.ea.receive.service.ReceiveService#updateApproval(java.util.Map)
	 * ml180117.ml07_T18 김진국 - ReceiveServiceImpl.java에 updateApprovalMustOpinion 구현 메서드를 만든다 - 결재 승인한 사람이든 결재 승인 안 한 사람이든 본인과 전 결재승인자가 있으면 모두 승인취소 해야한다.
	 */
	@Override
	@Transactional
	public void updateApprovalMustOpinion(ApprovalMasterVO approvalMasterVO, ReceiveVO receiveVO, ImplDeptEmpVO implDeptEmpVO, AnnualVO annualVO) {
		if(approvalMasterVO.getApproval_seq().indexOf("|") > -1){
			...
			for(int i= 0; i<approval_seqs.length; i++){
				...
				newReportDAO.updateApprovalMasterState(approvalMasterVO);
				receiveDAO.updateApprovalMustOpinion(receiveVO);
			}
		}else{
			/*승인*/
			if("1".equals(receiveVO.getGubun())){
