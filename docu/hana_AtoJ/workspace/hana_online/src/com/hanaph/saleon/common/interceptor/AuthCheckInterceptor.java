...
public class AuthCheckInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(usingAuth == null) {	//NoCheckLogin 어노테이션이 없음으로 무조건 로그인 체크
			...
			LoginUserVO loginUserVO = (LoginUserVO) session.getAttribute("onlineUser");
			if (loginUserVO != null) {
				throw new ModelAndViewDefiningException(CommonUtil.getMessageView("이미 로그인 사용자가 있습니다. 접속이 불가합니다. \n 기존 로그인을 해제하시고 재접속 하십시오. ", "Commons.sessionOut();", null));
			} // # ml.005 이미 기 사용자가 로그인 되어 있으면(세션 생성) 접속 불가하기 위한 새로운 제어문. 여기서 loginForm.jsp의
			// # ml.005 $.ajax의 오류 콜백으로 처리 if (error = "SyntaxError: Unexpected token < in JSON at position 0"){ // 김진국
