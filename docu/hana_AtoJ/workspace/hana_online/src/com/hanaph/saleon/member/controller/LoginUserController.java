...
public class LoginUserController {
    ...
    public void getLoginPassword(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        //2018-01-25 아래 IP로 접속하면 패스워드 체크 하지 않는다.
		String mngIpList[] = {"210.180.238.141","210.180.238.142","210.180.238.143"
				 ,"210.180.238.144","210.180.238.145","210.180.238.146"
				 ,"210.180.238.147","210.180.238.148","210.180.238.149"
				 ,"210.180.238.150","210.180.238.151","210.180.238.152"
				 ,"210.180.238.220","210.180.238.225","0:0:0:0:0:0:0:1"
				 };
		String getClientIp = request.getRemoteAddr();

		boolean managerLogin = false;
		System.out.println(getClientIp);
		for(int i = 0 ; i < mngIpList.length ; i++){
			if(mngIpList[i].equals(getClientIp)){
				managerLogin = true;
				break;
			}
		}
		...
		if(managerLogin){//패스워드 체크 안하는 로그인.
			
	        LoginUserVO loginUserVO = loginUserService.getLoginMng(paramMap);
            ...
            if (loginUserVO == null) {    //사원코드에 해당하는 사원이 없은 경우
    			 ...
    			 */
                if ("1".equals(loginUserVO.getEmp_gb()) || (!"1".equals(loginUserVO.getEmp_gb()) && loginUserVO.getSawon_id() != null && !loginUserVO.getSawon_id().equals(""))) {
                    if ("1".equals(loginUserVO.getEmp_gb()) && ("N".equals(loginUserVO.getUse_yn()))) {
	        ...
	        /**
	         * 2018-01-02 로그인 시도한 user 의 ELECTRONICSIGN 테이블의 PWD 값이 null 이면
	         * 변경 팝업 띄워준다
	         */
	        LoginUserVO firstLoginChk = loginUserService.getLoginFirstChk(paramMap);
	        
	        if(null != firstLoginChk){
	            ...
	            session.setMaxInactiveInterval(60 * 60);    // 세션 시간 설정 1시간
	            session.setAttribute("onlineUser", firstLoginChk);    // 유저정보를 onlineUser라는 이름으로 세션에 저장함.

	            
	        	
	            session.setAttribute("passChangeType", "first");        // 비밀번호 변경 팝업을 띄우게 설정
	            session.setAttribute("reqChgPassword", "true");        // 비밀번호 변경 팝업을 띄우게 설정
	            session.setAttribute("chgPasswordReason", "비밀번호를 설정해 주세요.");    // alert 메세지

				/*
				 *  메뉴 권한 조회
				 */
	            SessionUserInfoVO sessionUserInfoVO = (SessionUserInfoVO) sessionContextFactory.getObject();            //세션빈 객체를 생성
	            sessionUserInfoVO.setLoginUserVO(firstLoginChk);                                                        //세션빈 객체에 사원 정보를 저장함.
	            sessionUserInfoVO.setMenuList(mgmtProgramService.getMenuAuthByUser(firstLoginChk.getEmp_code()));        //세션빈의 menuList변수에 사원이 접근가능한 프로그램 정보를 셋팅함

	            firstLoginChk.setResultcode(1);    //로그인 성공

	        	
	            MarshallerUtil.marshalling("json", response, firstLoginChk);    //VO를 json으로 변경해서 뷰단에 응답함
	        }else{
	            /**
    ...
    public void updatePassword(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(( "first".equals(passChangeType) && "".equals(newPassword) ) || (!"first".equals(passChangeType) && "".equals(oldPassword) || "".equals(newPassword))){
        ...
        if ("0".equals(validateResultVO.getOut_CODE())) {
        	...
        	if("first".equals(passChangeType)){
        		loginUserVO = loginUserService.getLoginFirstChk(paramMap);
        	}else{
