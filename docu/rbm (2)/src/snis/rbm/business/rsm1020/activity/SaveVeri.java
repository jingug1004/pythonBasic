...
public class SaveVeri {
	...
	public String strVeriDTPTFst() {
		...
		//String sMeetCd = Util.getCtxStr(ctx, "MEET_CD"); // 경주일
		
		String sMeetGubun = Util.getCtxStr(ctx, "MEET_CD"); // 구분 MeetGubun
		String sJigeupDt = Util.getCtxStr(ctx, "RACE_DAY"); // 구분 MeetGubun
		String sMeetCd = Util.getCtxStr(ctx, "MEET_CD"); // 경주일
		
		if(sMeetGubun==null || sMeetGubun.equals("")) sMeetGubun = sMeetCd;
	...
	// 1차 검증 확인(TBJI_PC_SATLIT VW_SELL_DEV@CYBETLINK / RACE_NO,POOL_CD)
	public String strVeriDTVwGrnSell() {
		String rtnKey = "";

		//String sMeetGubun = (String) ctx.get("MEET_GUBUN");		// 구분 MeetGubun
		//String sJigeupDt  = Util.getCtxStr(ctx, "RACE_DAY");	// 경주일
		//String sMeetCd	  = Util.getCtxStr(ctx, "MEET_CD");		// 구분
		...
		PosRowSet keyRecord = this.rbmdao.find("rsm1020_s08", param);
		PosRow pr[] = keyRecord.getAllRow();
	...
	// 1차 검증 확인(TBJI_PC_SATLIT VW_SELL_DEV@CYBETLINK / COMM_NO)
	public String strVeriDTVwGrnSellDev() {
		String rtnKey = "";
		...
		PosRowSet keyRecord = this.rbmdao.find("rsm1020_s09", param);
		PosRow pr[] = keyRecord.getAllRow();
...
//		String SESSION_USER_ID = Util.trim(ctx.getSessionUserData("USER_ID".trim()));
		...
		String sJigeupDt = Util.getCtxStr(ctx, "RACE_DAY"); // 구분 MeetGubun
		String sMeetCd = Util.getCtxStr(ctx, "MEET_CD"); // 경주일
		String sStndYear = Util.getCtxStr(ctx, "STND_YEAR"); // 경주일
...
//		String sDT = (String) ctx.get("DT"); // 일자
	...
	public int insertVeriFstPcall(String sMeetCd, String SESSION_USER_ID, String sFile,String sFstVeri,String sSndVeri, String sVeri) {

		String sMeetGubun = sMeetCd;
		//String sRaceDay   = (String) ctx.get("RACE_DAY");
		String sRaceDay   = Util.getCtxStr(ctx, "RACE_DAY");
		String sStndYear  = sRaceDay.substring(0,4);
		String sDt 		  = sRaceDay.substring(4,8);
		...
		param.setValueParamter(i++, sDt); 		// 4.날짜
		param.setValueParamter(i++, sFile); 	// 5.파일 업로드 결과
