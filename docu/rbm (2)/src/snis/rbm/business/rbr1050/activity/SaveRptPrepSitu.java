...
public class SaveRptPrepSitu extends SnisActivity {
	
	public SaveRptPrepSitu(){}
    ...
    */
    ...
    {		
        ...
        param.setValueParamter(i++, record.getAttribute("WRKG_INFO"));        	// 5. 근무인원정보	
        param.setValueParamter(i++, record.getAttribute("DUTY_OFF_INFO"));      // 6. 휴가인원정보
        param.setValueParamter(i++, record.getAttribute("NUM_CHK_RSLT"));   	// 7. 인원 이상유무
        param.setValueParamter(i++, record.getAttribute("CHK_POINT"));   		// 8. 점검항목
        param.setValueParamter(i++, record.getAttribute("CHK_POINT_RSLT"));     // 9. 점검결과
        param.setValueParamter(i++, record.getAttribute("SIGNFT_POINT"));       //10. 특이사항
        param.setValueParamter(i++, SESSION_USER_ID);                          	//11. 등록/수정자 사번  
