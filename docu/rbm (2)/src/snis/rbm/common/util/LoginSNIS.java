...
public class LoginSNIS extends PosActivity {
	...
	private void loginUser(PosContext ctx) {
		...
		PosParameter preparam = new PosParameter();
		PosRowSet rowset;
		...
		if (rowset.hasNext()) {
			...
			i = 0;
			preparam.setWhereClauseParameter(i++, sUserId);
			//rowsetOther = this.getDao("rbmdao").find("tbrk_user_login_view_other_org", param);
			rowsetOther = this.getDao("rbmdao").find("tbrk_user_login_view_other_org", preparam);			
