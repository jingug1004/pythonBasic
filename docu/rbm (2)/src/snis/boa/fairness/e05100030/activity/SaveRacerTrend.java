 ...
 * 최종수정내용	: 
=================================================================================*/
package snis.boa.fairness.e05100030.activity;
...
*/
public class SaveRacerTrend extends SnisActivity
{   	
    public SaveRacerTrend()
    {
    }
    ...
    */
    protected void saveState(PosContext ctx) 
    {
        ...
        ds    = (PosDataset) ctx.get("dsOutRacerTrend");
        ...
        for ( int i = 0; i < nSize; i++ ) 
        {
            ...
            {
            	// MERGE
            	nSaveCount = nSaveCount + saveRacerTrend(record);
            }
        }
     ...
     * @throws  none
     */
    protected int saveRacerTrend(PosRecord record)
    {
    	int dmlcount   = 0; 
        ...
        param.setValueParamter(i++, Util.trim(record.getAttribute("CHK")));
        param.setValueParamter(i++, Util.trim(record.getAttribute("RMK")));
        param.setValueParamter(i++, SESSION_USER_ID);
        param.setValueParamter(i++, SESSION_USER_ID);
        	
        dmlcount = this.getDao("boadao").update("tbee_risk_mng_ie301", param);
