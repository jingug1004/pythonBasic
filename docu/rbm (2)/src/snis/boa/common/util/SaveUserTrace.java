...
public class SaveUserTrace extends SnisActivity {
	...
	public String runActivity(PosContext ctx) {
    	...
    	if ("P".equals(sJobType)) {	// 개인정보 접근 정보
			SavePersonalAccess(ctx);
    	} else {		// 비정상 종료시의 종료정보
    		FinishUserTraceLog(ctx);
		}
	 ...
	 * @throws none
	 */
	private void FinishUserTraceLog(PosContext ctx) {
		String sJobType = (String) ctx.get("JOB_TYPE".trim());
    ...
    protected int updateUserTrace(PosRecord record)
    {
        ...
        int dmlcount = this.getDao("boadao").update("user_trace_finish", param);
        return dmlcount;
    }
	...
	private void SavePersonalAccess(PosContext ctx) {
        ...
        int nSaveCount = this.getDao("boadao").update(sQueryId, param);
