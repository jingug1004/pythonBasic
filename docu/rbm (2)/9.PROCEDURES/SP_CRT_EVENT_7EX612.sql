CREATE OR REPLACE PROCEDURE USRBM.SP_CRT_EVENT_7EX612(P_CUR_DATE VARCHAR)
 IS
    ...
    AND    EXISTS  (
            ...
            FROM (
                    SELECT CARD_NO,
                             ...
                             NVL(SUM(CASE WHEN RACE_NO = '12' THEN 1 END),0) AS RACE_12_CNT
                    FROM TBRD_EVENT_7EX_DETL
                    WHERE EVENT_DT = P_CUR_DATE
                    AND     TO_CHAR(ATL_DATE, 'HH24MISS') <= V_END_TM
                    GROUP BY CARD_NO
                    ) B
            WHERE '|'||RACE_06_CNT||'|'||RACE_07_CNT||'|'||RACE_08_CNT||'|'||RACE_09_CNT||'|'||RACE_10_CNT||'|'||RACE_11_CNT||'|'||RACE_12_CNT||'|' NOT LIKE '%|0|%'
